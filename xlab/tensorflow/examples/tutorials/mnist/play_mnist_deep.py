...
def training(data, model, param):
    ...
    output_dim = np.size(data.train.next_batch(1)[1], 1)
    ...
    if len(param) == 0:
        param['W'] = tf.Variable(tf.zeros([784,10]))
        param['b'] = tf.Variable(tf.zeros([10]))
    else:
        param['W'] = tf.Variable(param['W'])
        param['b'] = tf.Variable(param['b'])
...
def training_adam_dropout(model, data):
    sess = tf.InteractiveSession()
...
def simple_model(x, param):
    return tf.matmul(x,param['W']) + param['b']
